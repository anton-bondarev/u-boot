...
static ulong xmodem_read_fit(struct spl_load_info *load, ulong offset,
			     ulong size, void *addr)
{
	int res, err, buf_offset;
	struct xmodem_fit_info *info = load->priv;
	char *buf = info->buf;
...
SPL_LOAD_IMAGE_METHOD("UART", 0, BOOT_DEVICE_UART, spl_xmodem_load_image);
