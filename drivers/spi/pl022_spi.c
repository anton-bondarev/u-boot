...
static int pl022_spi_probe(struct udevice *bus)
{
	...
	/* 8 bits per word, high polarity and default clock rate 0x04C7*/
	writel(SSP_CR0_SPO | SSP_CR0_SPH | SSP_CR0_8BIT_MODE, ps->base + SSP_CR0);
	writel(DFLT_PRESCALE, ps->base + SSP_CPSR);
