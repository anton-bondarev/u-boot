...
struct mdma_chan *mdma_chan_create(void *base_addr, struct mdma_cfg *cfg)
{
	...
	chan = malloc(sizeof(struct mdma_chan));
	if (!chan) {
		chan_debug("\tChannel create: Unable allocate memory.\n");
		goto chan_create_exit;
	}
	...
	chan_print_dbg(0, "channel(0x%p) ---> base address(0x%p)", chan, chan->regs);
...
int mdma_chan_attach(struct mdma_chan *master, struct mdma_chan *slave)
{
	...
	chan_print_dbg(0, "channel(0x%p) ---> channel(0x%p)", master, master->slave);
...
void mdma_chan_destroy(struct mdma_chan *chan)
{
	...
	chan_print_dbg(0, "channel(0x%p)", chan);
...
int mdma_trans_create(struct mdma_chan *chan, enum mdma_trans_type trans_type)
{
	...
	trans = malloc(sizeof(struct mdma_trans));
	if (!trans) {
		chan_debug("\tTransfer create: Unable allocate memory.\n");
	...
	chan_print_dbg(0, "channel(0x%p) ---> transfer(0x%p)", chan, chan->trans);
...
void mdma_trans_destroy(struct mdma_chan *chan)
{
	...
	chan_print_dbg(0, "channel(0x%p) ---x transfer(0x%p)", chan, trans);
			...
			int custom_flags, bool is_last)
{
	...
	if ((cfg->burst_width != MDMA_BURST_WIDTH4) &&
		...
		chan_debug("\tAdd group: Uncorrectable burst width.\n");
	...
	chan_print_dbg(0, "transfer(0x%p) ---> memory address(0x%p)", trans, mem_addr);
	chan_print_dbg(0, "transfer(0x%p) ---> memory size = 0x%x", trans, mem_size);
	...
	if (cfg->desc_kind == NORMAL_DESC) {
		...
		chan_print_dbg(0, "transfer(0x%p) ---> string length = 0x%x", trans, cfg->str_length);
		chan_print_dbg(0, "transfer(0x%p) ---> pitch width = 0x%x", trans, cfg->pitch_width);
		...
		chan_debug("\tAdd group: Unknown descriptor kind.\n");
	...
	chan_print_dbg(0, "transfer(0x%p) ---> data size limit = 0x%x, number of data nodes = 0x%x",
			trans, size_limit, num_node);
	
	group = malloc(sizeof(struct mdma_group));
	if (!group) {
		chan_debug("\tAdd group: Unable allocate memory.\n");
	...
	if (!is_last)
		...
		chan_debug("\tAdd group: Unknown transfer type.\n");
		free(group);
	...
	group->node_group = malloc(group->node_nmb * sizeof(struct mdma_node));
	...
	group->desc_pool_addr = memalign(desc_align, group->desc_pool_size);
	if (group->desc_pool_addr == NULL) {
		chan_debug("\tAdd group: Unable allocate descriptors pool size = 0x%x.\n",
				group->desc_pool_size);
		free(group->node_group);
		free(group);
	...
	chan_print_dbg(0, "group(0x%p) ---> nodes pool address(0x%p), number of nodes = 0x%x",
			group, group->node_group, group->node_nmb);
	chan_print_dbg(0, "group(0x%p) ---> descriptors pool address(0x%p), size of descriptor = 0x%x",
			group, group->desc_pool_addr, group->gap_size);
	
	if (trans->first_group == NULL) {
		trans->first_group = group;
		chan_print_dbg(0, "transfer(0x%p) ---> first group address(0x%p)", trans, group);
	}
	
	if (is_last) {
		trans->last_group = group;
		chan_print_dbg(0, "transfer(0x%p) ---> last group address(0x%p)", trans, group);
	}
	...
	for (i = 0; i < num_node; i++) {
		...
		chan_print_dbg(0, "group(0x%p) ---> data node address(0x%p), index = 0x%x",
		...
		chan_print_dbg(0, "node(0x%p) ---> data address(0x%p), data size = 0x%x",
	...
	if (i < group->node_nmb) {
		...
		chan_print_dbg(0, "group(0x%p) ---> link node address(0x%p), index = 0x%x",
		...
		if (is_last) {
			...
			chan_print_dbg(0, "node(0x%p) is last node of ring, link at address(0x%p)",
					node, node->mem_addr);
		}
	}
	else {
		...
		chan_print_dbg(0, "node(0x%p) is last node of queue", node);
	}
	...
	if (!list_empty(&trans->group_list)) {
		...
		chan_print_dbg(0, "group(0x%p) ---> node(0x%p) is link at address(0x%p)",
	...
	chan_print_dbg(0, "transfer(0x%p) ---> group(0x%p)", trans, group);
...
void mdma_trans_free_groups(struct mdma_chan *chan)
{
	...
	chan_print_dbg(0, "channel(0x%p) ---> transfer(0x%p)", chan, trans);

	mdma_chan_reset(chan);

	list_for_each_entry_safe(group, next, &trans->group_list, group_node) {
		chan_print_dbg(0, "transfer(0x%p) ---x group(0x%p)", trans, group);
		...
		for (i = 0; i < group->node_nmb; i++) {
			...
			chan_print_dbg(0, "group(0x%p) ---x node(0x%p)", group, node);
		...
		chan_print_dbg(0, "group(0x%p) ---x descriptor poll(0x%p)",
				group, group->desc_pool_addr);

		free(group->desc_pool_addr); // allocate from specific heap
...
static int chan_set_desc(struct mdma_node *node, struct mdma_cfg *cfg)
{
	...
	case NORMAL_DESC:
		do {
			...
			chan_print_dbg(0, "node(0x%p) settings to normal descriptor(0x%p), index = 0x%x",
	...
	case LONG_DESC:
		do {
			...
			chan_print_dbg(0, "node(0x%p) settings to long descriptor(0x%p), index = 0x%x",
					node, long_desc, node->desc_indx);

			iowrite32(0, (volatile void*)(&long_desc->usrdata_l));
			iowrite32(0, (volatile void*)(&long_desc->usrdata_h));
	...
	case PITCH_DESC:
		do {
			...
			chan_print_dbg(0, "node(0x%p) settings to pitch descriptor(0x%p), index = 0x%x",
					node, pitch_desc, node->desc_indx);

			iowrite32(0, (volatile void*)(&pitch_desc->usrdata));
...
int mdma_chan_config(struct mdma_chan *chan)
{
	...
	if ((cfg->mode != MDMA_CHAN_INTERRUPT) &&
		...
		chan_debug("\tChannel configure: Unknown channel mode.\n");

		ret = -3;
		goto chan_config_exit;
	}

	chan_print_dbg(0, "channel(0x%p) ---> transfer(0x%p), number of groups = 0x%x",
			chan, trans, trans->group_nmb);

	list_for_each_entry(group, &trans->group_list, group_node) {
		chan_print_dbg(0, "transfer(0x%p) ---> group(0x%p), number of nodes = 0x%x",
	...
	if (!(cfg->event_indx < 0)) {
		...
		if (cfg->sync_mode == MDMA_SYNC_SLAVE)
			...
			chan_debug("\tChannel configure: Unknown synchronization mode.\n");
		...
		{
			MDMA_CHAN_BUG("channel(0x%p) - unknown event prior", chan);
		}
	...
	chan_print_dbg(0, "channel(0x%p) - state = 0x%x", chan, chan->state);
...
int mdma_chan_reset(struct mdma_chan *chan)
{
	...
	while (mdma_chan_read32((size_t)(&chan->regs->enable)) & MDMA_ENABLE) {
		if(!timeout)
		{
			MDMA_CHAN_BUG("channel(0x%p) - reset timeout", chan);
		}
		else
			timeout--;
	...
	if (mdma_chan_read32((size_t)(&chan->regs->cancel)) & MDMA_CANCEL)
		MDMA_CHAN_BUG("channel(0x%p) - reset error", chan);
...
static int chan_is_error(struct mdma_chan *chan, uint32_t status)
{
	...
	chan_print_dbg(1, "channel(0x%p) - state = 0x%x, error = 0x%x",
...
static int chan_check_event(struct mdma_chan *chan, uint32_t status)
{
	...
	if (status & MDMA_STATUS_START_EVENT) {
		...
		if (event_active & ~(1 << chan->cfg.event_prior))
			MDMA_CHAN_BUG("channel(0x%p) - unknown event", chan);
	...
	chan_print_dbg(0, "channel(0x%p) - state = 0x%x, event_active = 0x%x, event_ignore = 0x%x",
...
static int chan_trans_progress(struct mdma_chan *chan, int budget)
{
	...
	chan_print_dbg(0, "channel(0x%p) ---> transfer(0x%p) [current group(0x%p), next group(0x%p)]",
			chan, trans, trans->cur_group, trans->next_group);
	
	if (!budget || (trans->next_group == NULL))
		goto progress_done;

	chan_print_dbg(0, "transfer(0x%p) - number of group to handling = 0x%x, budget = 0x%x",
	...
	do {
		...
		if (((size_t)desc_flags) & 0x3)
			MDMA_CHAN_BUG("channel(0x%p) - unaligned address = 0x%p", chan, desc_flags);
		...
		chan_print_dbg(0, "transfer(0x%p) ---> next group(0x%p)", trans, trans->next_group);
	} while (budget);
...
state_exit:
	chan_print_dbg(0, "channel(0x%p) - state = 0x%x", chan, ret);
...
int mdma_chan_irq_cb(struct mdma_chan *chan)
{
	...
	status &= mask;
	if (!status)
		MDMA_CHAN_BUG("channel(0x%p) - unknown IRQ", chan);
...
cb_exit:
	chan_print_dbg(0, "channel(0x%p) - state = 0x%x, irq = 0x%x",
			chan, chan->state, ret);
...
int mdma_chan_start(struct mdma_chan *chan)
{
	...
	if (!(cfg->event_indx < 0)) {
		...
		if (val)
			chan_debug("channel(0x%p): discard ignore_events - 0x%x.\n", chan, val);

		val = mdma_chan_read32((size_t)(&chan->regs->activ_events));
		mdma_chan_write32(0, (size_t)(&chan->regs->activ_events));
		if (val)
			chan_debug("channel(0x%p): discard active_events - 0x%x.\n", chan, val);
	}

	val = mdma_chan_read32((size_t)(&chan->regs->status));
	if (val)
		chan_debug("channel(0x%p): discard IRQ, status - 0x%x.\n", chan, val);
...
int mdma_chan_pause(struct mdma_chan *chan, bool suspend)
{
	...
	while (mdma_chan_read32((size_t)(&chan->regs->dma_state)) & mask) {
		if (!timeout)
		{
			MDMA_CHAN_BUG("channel(0x%p) - pause timeout", chan);
		}
		else
			timeout--;
...
struct mdma_trans *mdma_chan_get_trans(struct mdma_chan *chan)
{
	...
	chan_print_dbg(1, "channel(0x%p) ---> transfer(0x%p)", chan, chan->trans);
...
int mdma_trans_ignore_event(struct mdma_trans *trans)
{
	...
	if (trans->ignore_event) {
		trans->ignore_event = false;
		chan_print_dbg(0, "transfer(0x%p)- ignore event", trans);
...
struct mdma_group *mdma_trans_get_group(struct mdma_trans *trans)
{
	...
	if (!group)
		MDMA_CHAN_BUG("transfer(0x%p) - NULL pointer", trans);
...
int mdma_trans_rest_group(struct mdma_group *group)
{
	...
	chan_print_dbg(0, "transfer(0x%p) ---> group(0x%p)", group->trans, group);
...
struct mdma_node *mdma_group_get_node(struct mdma_group *group)
{
	...
	chan_print_dbg(0, "group(0x%p) ---> node(0x%p), index = 0x%x", group, node, group->cur_indx);
			...
			void *mem_addr, size_t mem_size)
{
	...
	chan_print_dbg(0, "node(0x%p), index = 0x%x ---> memory address(0x%p), size = 0x%x",
...
void *mdma_node_get_desc(struct mdma_node *node, int *size)
{
	...
	chan_print_dbg(0, "channel(0x%p)", group->trans->chan);
	
	desc = group->desc_pool_addr + group->gap_size * node->desc_indx;
	*size = group->gap_size;

	chan_print_dbg(0, "node(0x%p) ---> descriptor(0x%p), index = 0x%x", node, desc, node->desc_indx);
...
uint32_t mdma_desc_flags_length(void *desc, int size)
{
	...
	chan_print_dbg(0, "descriptor(0x%p)", desc);
