...
struct rcm_1888tx018_mmc_platdata {
...
static inline struct rcm_1888tx018_mmc_platdata * mmc_get_platdata(const struct mmc * mmc)
{
	return (struct rcm_1888tx018_mmc_platdata *) mmc->priv;
}
...
#ifdef DEBUG_REG
static void rcm_1888tx018_print_op(char * func, uint32_t adr, int reg, uint val)
{
	static int s_reg = -1;
	static uint s_val;
	static bool flag;
...
#else
	#define rcm_1888tx018_print_op(func, adr, reg, val)
#endif
...
static inline void rcm_1888tx018_writel(const struct mmc * mmc, int reg, u32 val)
{
	uint32_t regbase = mmc_get_platdata(mmc)->reg_base;
	rcm_1888tx018_print_op("rcm_1888tx018_writeL", regbase, reg, val);
	writel(val, regbase + reg);
}
static inline void rcm_1888tx018_writew(const struct mmc * mmc, int reg, u16 val)
{
	uint32_t regbase = mmc_get_platdata(mmc)->reg_base;
	rcm_1888tx018_print_op("rcm_1888tx018_writeW", regbase, reg, val);
	writew(val, regbase + reg);
}
static inline void rcm_1888tx018_writeb(const struct mmc * mmc, int reg, u8 val)
{
	uint32_t regbase = mmc_get_platdata(mmc)->reg_base;
	rcm_1888tx018_print_op("rcm_1888tx018_writeB", regbase, reg, val);
	writeb(val, regbase + reg);
}

static inline u32 rcm_1888tx018_readl(const struct mmc * mmc, int reg)
{
	uint32_t regbase = mmc_get_platdata(mmc)->reg_base;
	u32 retval = readl(regbase + reg);
	rcm_1888tx018_print_op("rcm_1888tx018_readL", regbase, reg, retval);
	return retval;
}
static inline u16 rcm_1888tx018_sdhci_readw(const struct mmc * mmc, int reg)
{
	uint32_t regbase = mmc_get_platdata(mmc)->reg_base;
	u16 retval = readw(regbase + reg);
	rcm_1888tx018_print_op("rcm_1888tx018_readW", regbase, reg, retval);
	return retval;
}
static inline u8 rcm_1888tx018_readb(const struct mmc * mmc, int reg)
{
	uint32_t regbase = mmc_get_platdata(mmc)->reg_base;
	u8 retval = readb(regbase + reg);
	rcm_1888tx018_print_op("rcm_1888tx018_readB", regbase, reg, retval);
	return retval;
}
...
static bool wait_cmd_done_handle(const struct mmc * mmc) 
{
	...
	do {
		int status = rcm_1888tx018_readl(mmc, SPISDIO_SDIO_INT_STATUS);
		if ( status & SPISDIO_SDIO_INT_STATUS_CAR_ERR ) {
			rcm_1888tx018_writel(mmc, SDIO_SDR_BUF_TRAN_RESP_REG, SDR_TRAN_SDC_ERR);
			Debug(" ECMD<0x%x>\n", status);
			return false;
		} else if ( status & SPISDIO_SDIO_INT_STATUS_CMD_DONE ) {
			rcm_1888tx018_writel(mmc, SDIO_SDR_BUF_TRAN_RESP_REG, SDR_TRAN_SDC_CMD_DONE);
			return true;
		}
...
static bool wait_tran_done_handle(const struct mmc * mmc)
{
	...
	do {
		...
		if ( status & SPISDIO_SDIO_INT_STATUS_CAR_ERR ) {
			...
			rcm_1888tx018_writel(mmc, SDIO_SDR_BUF_TRAN_RESP_REG, SDR_TRAN_SDC_DAT_DONE);
			return true;
		}
	} while ( wait_tick() - start < SDIO_TIMEOUT );
...
static bool wait_ch0_dma_done_handle(const struct mmc * mmc)
{
	...
	do {
		...
		if ( status & SPISDIO_SDIO_INT_STATUS_CAR_ERR ) {
			...
			rcm_1888tx018_writel(mmc, SDIO_DCCR_0, DSSR_CHANNEL_TR_DONE);
			return true;
		}
	} while ( wait_tick() - start < SDIO_TIMEOUT );
...
static bool wait_ch1_dma_done_handle(const struct mmc * mmc)
{
	...
	do {
		...
		if ( status & SPISDIO_SDIO_INT_STATUS_CAR_ERR ) {
			...
			rcm_1888tx018_writel(mmc, SDIO_DCCR_1, DSSR_CHANNEL_TR_DONE);
			return true;
		}
	} while ( wait_tick() - start < SDIO_TIMEOUT );
...
static bool wait_buf_tran_finish_handle(const struct mmc * mmc)
{
	...
	do {
		...
		if ( status & SPISDIO_SDIO_INT_STATUS_CAR_ERR ) {
			...
			rcm_1888tx018_writel(mmc, SDIO_SDR_BUF_TRAN_RESP_REG, SDR_TRAN_FIFO_FINISH);
			return true;
		}
	} while( wait_tick() - start < SDIO_TIMEOUT );
...
static bool sd_send_cmd(const struct mmc * mmc, uint32_t cmd_ctrl, uint32_t arg)
{
	...
	rcm_1888tx018_writel(mmc, SDIO_SDR_CMD_ARGUMENT_REG, arg);
	rcm_1888tx018_writel(mmc, SDIO_SDR_CTRL_REG, cmd_ctrl);
...
{
	rcm_1888tx018_writel(mmc, SDIO_SDR_ADDRESS_REG, (buf_num << 2));
	rcm_1888tx018_writel(mmc, SDIO_SDR_CARD_BLOCK_SET_REG, (len == 512 ? BLOCK_512_DATA_TRANS : (len << 16) | 0x0001));
	rcm_1888tx018_writel(mmc, SDIO_SDR_CMD_ARGUMENT_REG, idx);
	rcm_1888tx018_writel(mmc, SDIO_SDR_CTRL_REG, cmd_ctrl | mmc_get_platdata(mmc)->bus_width);
...
{
	rcm_1888tx018_writel(mmc, SDIO_BUF_TRAN_CTRL_REG, BUFER_TRANS_START | (dir == DATA_WRITE ? BUFER_TRANS_WRITE : 0x0) | buf_num);
	rcm_1888tx018_writel(mmc, (dir == DATA_WRITE ? SDIO_DCDTR_0 : SDIO_DCDTR_1), len);
	rcm_1888tx018_writel(mmc, (dir == DATA_WRITE ? SDIO_DCSSAR_0 : SDIO_DCDSAR_1), (uint32_t) mem_ptr);
	rcm_1888tx018_writel(mmc, (dir == DATA_WRITE ? SDIO_DCCR_0 : SDIO_DCCR_1), (dir == DATA_WRITE ? DCCR_VAL_WRITE : DCCR_VAL_READ));
...
{
	rcm_1888tx018_writel(mmc, SDIO_SDR_CARD_BLOCK_SET_REG, (len == 512 ? BLOCK_512_DATA_TRANS : (len << 16) | 0x0001));
	rcm_1888tx018_writel(mmc, SDIO_BUF_TRAN_CTRL_REG, BUFER_TRANS_START | BUFER_TRANS_WRITE | 0);
	rcm_1888tx018_writel(mmc, SDIO_DCDTR_0, len);
	rcm_1888tx018_writel(mmc, SDIO_DCSSAR_0, (uint32_t) src_ptr);
	rcm_1888tx018_writel(mmc, SDIO_DCCR_0, DCCR_VAL_WRITE);
	...
	rcm_1888tx018_writel(mmc, SDIO_SDR_ADDRESS_REG, (0 << 2));
	rcm_1888tx018_writel(mmc, SDIO_SDR_CARD_BLOCK_SET_REG, (len == 512 ? BLOCK_512_DATA_TRANS : (len << 16) | 0x0001));
	rcm_1888tx018_writel(mmc, SDIO_SDR_CMD_ARGUMENT_REG, dst_adr);
	rcm_1888tx018_writel(mmc, SDIO_SDR_CTRL_REG, CMD24_CTRL | mmc_get_platdata(mmc)->bus_width);
...
static int rcm_1888tx018_dm_mmc_get_cd(struct udevice * dev)
{
	struct rcm_1888tx018_mmc_platdata * pdata = dev_get_platdata(dev);

	bool carddetected = (dm_gpio_get_value(& pdata->card_detect) == 0);
	Debug(">rcm_1888tx018_dm_mmc_get_cd: %d\n", carddetected);

	return carddetected ? 1 : 0;
}

static int rcm_1888tx018_dm_mmc_get_wp(struct udevice * dev)
{
	Debug(">rcm_1888tx018_dm_mmc_get_cd:\n");

	return 0;
}

static int rcm_1888tx018_mmc_probe(struct udevice * dev)
{
	Debug(">rcm_1888tx018_mmc_probe\n");
	...
	rcm_1888tx018_writel(mmc, SPISDIO_ENABLE, 0x1);  //sdio-on, spi-off

	/* Hard reset damned hardware */
	rcm_1888tx018_writel(mmc, SDIO_SDR_CTRL_REG, 0x1 << 7);    
	while ( rcm_1888tx018_readl(mmc, SDIO_SDR_CTRL_REG) & (0x1 << 7) )
		;

	//!!! do we really need it?
	//write_PL022_IMSC(GSPI__, 0x0);//disable interrupts in SSP

	rcm_1888tx018_writel(mmc, SPISDIO_SDIO_INT_MASKS, 0x7F);  //enable  interrupts in SDIO

	rcm_1888tx018_writel(mmc, SPISDIO_SPI_IRQMASKS, 0x00);  //disable interrupts from ssp

	rcm_1888tx018_writel(mmc, SDIO_SDR_ERROR_ENABLE_REG, 0x16F);  //enable interrupt and error flag

	return 0;
}

static int rcm_1888tx018_mmc_ofdata_to_platdata(struct udevice *dev)
{
	Debug(">rcm_1888tx018_mmc_ofdata_to_platdata\n");	
...
int rcm_1888tx018_mmc_bind(struct udevice *dev)
{
	Debug(">rcm_1888tx018_mmc_bind\n");
...
static void set_clock(struct udevice * dev, uint clock)
{
	...
	rcm_1888tx018_writel(mmc, SPISDIO_SDIO_CLK_DIVIDE, m);  
}

static int rcm_1888tx018_dm_mmc_set_ios(struct udevice * dev)
{
	struct rcm_1888tx018_mmc_platdata * pdata = dev_get_platdata(dev);	
	struct mmc * mmc = mmc_get_mmc_dev(dev);
	Debug(">>rcm_1888tx018_dm_mmc_set_ios: clock=%d, bus_width=%d\n", mmc->clock, mmc->bus_width);
...
static void rcm_1888tx018_mmc_read_response(struct mmc *mmc, struct mmc_cmd *cmd)
{
	if (cmd->resp_type & MMC_RSP_136) {
		cmd->response[0] = rcm_1888tx018_readl(mmc, SDIO_SDR_RESPONSE4_REG);
		cmd->response[1] = rcm_1888tx018_readl(mmc, SDIO_SDR_RESPONSE3_REG);
		cmd->response[2] = rcm_1888tx018_readl(mmc, SDIO_SDR_RESPONSE2_REG);
		cmd->response[3] = rcm_1888tx018_readl(mmc, SDIO_SDR_RESPONSE1_REG);
		Debug(" {0x%x, 0x%x, 0x%x, 0x%x}\n", cmd->response[0], cmd->response[1], cmd->response[2], cmd->response[3]);
	} else {
		cmd->response[0] = rcm_1888tx018_readl(mmc, SDIO_SDR_RESPONSE1_REG);
		Debug(" {0x%x}\n", cmd->response[0]);
	}
}
...
static int rcm_1888tx018_dm_mmc_send_cmd(struct udevice *dev, struct mmc_cmd *cmd, struct mmc_data *data)
{
	struct mmc * mmc = mmc_get_mmc_dev(dev);
	int ret = 0;
	...
	if ( is_int_data_cmd ) {
		...
		rcm_1888tx018_writel(mmc, SDIO_SDR_ADDRESS_REG, (0 << 2));
		rcm_1888tx018_writel(mmc, SDIO_SDR_CARD_BLOCK_SET_REG, (data_len << 16) | 0x0001);
	}
	...
	rcm_1888tx018_mmc_read_response(mmc, cmd);
	
	return ret;
}

static const struct udevice_id rcm_1888tx018_mmc_match[] = {
	{ .compatible = "rcm,mmc-0.2" },
	{ }
};

static const struct dm_mmc_ops rcm_1888tx018_dm_mmc_ops = {
	.send_cmd = rcm_1888tx018_dm_mmc_send_cmd,
	.set_ios = rcm_1888tx018_dm_mmc_set_ios,
	.get_cd = rcm_1888tx018_dm_mmc_get_cd,
	.get_wp = rcm_1888tx018_dm_mmc_get_wp
};

U_BOOT_DRIVER(rcm_1888tx018_mmc_drv) = {
	.name = "rcm_1888tx018_mmc_drv",
	.id = UCLASS_MMC,
	.of_match = rcm_1888tx018_mmc_match,
	.ops = & rcm_1888tx018_dm_mmc_ops,
	.probe = rcm_1888tx018_mmc_probe,
	.bind = rcm_1888tx018_mmc_bind,
	.ofdata_to_platdata = rcm_1888tx018_mmc_ofdata_to_platdata,
	.platdata_auto_alloc_size = sizeof(struct rcm_1888tx018_mmc_platdata),
